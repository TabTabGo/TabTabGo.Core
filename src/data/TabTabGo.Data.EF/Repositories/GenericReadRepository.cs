...
public class GenericReadRepository<TEntity,TKey>: IDisposable, IGenericReadRepository<TEntity,TKey> where TEntity : class
{
    ...
    public GenericReadRepository(DbContext dbContext)
    {
       ...
       #region Get
        public async Task<PageList<TResult>> GetPageListAsync<TResult>(Func<TEntity, TResult> selector,
            ...
            , CancellationToken cancellationToken = default)
        {
            ...
            return filter != null ? query.CountAsync(filter, cancellationToken: cancellationToken) : query.CountAsync(cancellationToken: cancellationToken);
        }
        ...
        public virtual Task<int> ExecuteSqlCommand(string query, IDictionary<string, object>? parameters = null,
            CancellationToken cancellationToken = default)
        {
            return _dbContext.Database.ExecuteSqlRawAsync(query, parameters ?? new Dictionary<string,object>(), cancellationToken);
        }


        public virtual IQueryable<TEntity?> IncludeProperties(IQueryable<TEntity?> queryable, string[]? includeProperties = null)
        {
